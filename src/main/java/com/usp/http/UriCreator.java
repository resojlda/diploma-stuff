...
public class UriCreator {
    ...
    public String userSearchUri(Params params, String[] fields, OAuthToken authToken) {
        return METHOD_URI + "users.search?" +
                params.build() + "&" +
                "fields=" + StringUtils.join(fields, ",") + "&" +
                "access_token=" + authToken.getAccessToken();
    }
    ...
    static class Params {
        ...
        public String build() {
            ...
            params.keySet().stream().forEach(key -> result.append(key).append('=').append(params.get(key)).append('&'));
